...
// Project: Top 5 Moving Violations - September 2025 ONLY
// Purpose: Show top 5 officers for moving violations in September 2025
...
let
    ...
    // Filter: September 2025, Individual tickets only, Moving violations
    ...
    #"Grouped by Officer" = Table.Group(
        ...
        {{"Moving_Count", each Table.RowCount(_), Int64.Type}}
    ),

    // Sort descending
    #"Sorted" = Table.Sort(#"Grouped by Officer", {{"Moving_Count", Order.Descending}}),
    ...
    #"Final" = Table.RenameColumns(
        Table.SelectColumns(
            #"Added Rank",
            {"Rank", "OFFICER_DISPLAY_NAME", "PADDED_BADGE_NUMBER", "WG2", "TEAM", "Moving_Count"}
        ),
        {
            {"OFFICER_DISPLAY_NAME", "Officer"},
            {"PADDED_BADGE_NUMBER", "Badge"},
            {"WG2", "Bureau"},
            {"Moving_Count", "Moving Summons"}
        }
    )
in
    #"Final"
