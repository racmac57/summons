...
def main():
    ...
    try:
        ...
        # Use SUMMONS_CURRENT_MONTH sheet (26,042 records)
        target_sheet = 'SUMMONS_CURRENT_MONTH'
    ...
    try:
        # Find badge column (updated for SUMMONS_CURRENT_MONTH)
        badge_col = None
        for col in summons_df.columns:
            if any(keyword in str(col).upper() for keyword in ['BADGE', 'BADGE NUMBER']):
                badge_col = col
                break
